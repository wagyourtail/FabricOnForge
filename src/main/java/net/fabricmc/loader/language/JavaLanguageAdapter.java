...
public class JavaLanguageAdapter implements LanguageAdapter {
	private static boolean canApplyInterface(String itfString) throws IOException {
		...
		InputStream stream = FabricLoaderEarlyRiser.class.getResourceAsStream(className);
	...
	public static Class<?> getClass(String className, Options options) throws ClassNotFoundException, IOException {
		...
		InputStream stream = FabricLoaderEarlyRiser.class.getResourceAsStream(classFilename);
		...
		stream.close();
		return Class.forName(className, true, FabricLoaderEarlyRiser.classLoader);
	}
